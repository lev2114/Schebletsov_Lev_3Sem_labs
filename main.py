...
def task3(documents, sections, doc_sections):
    result = []

    section_dict = {}
    for ds in doc_sections:
        section_dict.setdefault(ds.section_id, []).append(ds.document_id)
    ...
    for section in sections:
        if section.id in section_dict and "раздел" in section.name.lower():
            ...
            result.append((section.name, docs))
...
def main():
    ...
    result3 = task3(documents, sections, DocSections)
    print("Задание 3: Разделы, содержащие слово 'раздел', и документы в них:")
    for section, docs in result3:
        print(section)
        for doc in docs:
            print("\t", doc)
